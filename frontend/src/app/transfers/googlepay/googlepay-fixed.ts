...
export class Googlepay implements OnInit {
  products: Product[] = [];
  loading = false;
  currentPage = 1;
  pageSize = 20;
  ...
  loadProducts() {
    ...
    const filters = { 
      categoryId: 'googlepay-transfers',
      isActive: true 
    };
    ...
    this.productService.getProducts(this.currentPage, this.pageSize, filters).subscribe({
      next: (response) => {
        ...
        this.loading = false;
        console.log('Loaded googlepay transfers products from backend:', response.products);
      },
      error: (error) => {
        console.error('Error loading googlepay transfers products:', error);
